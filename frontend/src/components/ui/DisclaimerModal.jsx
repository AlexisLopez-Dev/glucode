  ...
  return (
    ...
    >
      <div className="bg-surface rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col transform transition-all">
        ...
        {/* Cabecera */}
        <div className="p-5 border-b border-border-subtle flex justify-between items-center bg-warning-surface">
          <h3 className="font-bold text-warning-strong flex items-center gap-2">
            <span className="text-xl"><IconWarning /></span> Aviso Médico Importante
          </h3>
          <button
            type="button"
            onClick={onClose}
            className="text-warning-muted hover:text-warning-text transition-colors text-xl font-bold p-1"
          >
            ✕
          </button>
        </div>
        ...
        {/* Contenido */}
        <div className="p-6 text-sm text-text-secondary space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar">
          <p>
          ...
          <div className="pt-2 border-t border-border">
            <p className="font-bold text-text-strong mb-1">Responsabilidad del Usuario:</p>
            <p>
        ...
        <div className="p-4 border-t border-border-subtle bg-surface-alt flex justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2.5 bg-control-secondary text-text-secondary rounded-lg font-bold hover:bg-border-strong transition-colors"
          >
            Cancelar
          </button>
          <button
            type="button"
            onClick={onAccept}
            className="px-5 py-2.5 bg-primary text-on-primary rounded-lg font-bold hover:bg-primary-dark transition-colors shadow-sm"
          >
            He leído y acepto
          </button>
        </div>
