...
export const Sidebar = ({ isOpen, closeSidebar }) => {
  return (
    <>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 bg-overlay-sidebar backdrop-blur-sm z-30 md:hidden transition-opacity"
          onClick={closeSidebar}
        />
      )}

      {/* Sidebar */}
      <aside className={`w-64 fixed md:static inset-y-0 left-0 z-40 bg-surface border-r border-border flex flex-col shadow-sm transition-all duration-300 ease-in-out overflow-hidden shrink-0
      ...
      `}>
        ...
        <div className="h-24 flex items-center justify-center border-b border-transparent shrink-0">
          <Link 
            ...
            title="Ir al inicio"
          >
            <h1 className={`font-black text-text-strong tracking-tighter transition-all whitespace-nowrap ${isOpen ? 'text-4xl' : 'text-3xl'}`}>
              {isOpen ? <>Glucode<span className="text-primary">.</span></> : <>G<span className="text-primary">.</span></>}
            </h1>
          </Link>
        </div>
